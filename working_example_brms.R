...
stanvars <- stanvar(scode = stan_funs, block = "functions") +
  stanvar(x = as.numeric(testd$response) - 1, 
          name = "resp", scode = "real resp[N];")

make_stancode(rt ~ 1, data = testd, stanvars = stanvars, 
              family = ddm)
str(make_standata(rt ~ 1, data = testd, stanvars = stanvars, 
                  family = ddm))

fit1 <- brm(rt ~ 1, data = testd, stanvars = stanvars, 
            family = ddm)
fit2 <- brm(rt ~ 1, data = testd, stanvars = stanvars, 
            family = ddm)

saveRDS(fit1, file = file.path(getwd(), "brms_fit1.RDS"))
saveRDS(fit2, file = file.path(getwd(), "brms_fit2.RDS"))
## fit speed_acc data from rtdists
...
posterior_predict_ddm <- function(i, prep, ...) {
  ...
  sv <- brms::get_dpar(prep, "sv", i = i)
  resp <- prep$data$resp[i]
  rtdists::rdiffusion(n = length(resp), a = a, v = mu, t0 = ndt, z = w * a, sv = sv)
}
pp_check(fit1)
