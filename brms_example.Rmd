...
---
```{r echo=FALSE}
req_suggested_packages <- c("brms", "rtdists", "fddm")
...
Now that we've got our custom family ready in `brms`, we'll do a basic parameter recovery to show that everything works as expected.
...
The `loo` function can compare two models; in our case, we will use it to compare the 5-parameter DDM against the 4-parameter DDM.

First we load the 4-parameter fit (done analogously to the 5-parameter version in file `parameter_recovery_4_par.R`).
...
Before we run loo, we need to define another log-likelihood function for the 4-parameter DDM.
...
Based on the output of the `loo` function, we can see that the 5-parameter model has an ever so slightly higher estimate for ELPD than the 4-parameter model. However, the standard error on the difference between the ELPD estimates is much larger than the actual difference between the ELPD estimates. Therefore, `loo` suggests that these models perform equally well for the generated data.

Last, we can use the posterior predictive function that [we defined earlier](#brms-pp) to run posterior-predictive checking.
...
```{r rw-prep, eval=FALSE}

```

more text.

```{r rw-sampling, eval=FALSE, warning=FALSE}
